    ...
    c_BT_Iterator<T> operator++(int){
        ...
        do{
            ...
            if(m_pair.second==m_id){
                ...
                if(m_pair.first->m_child[0]) m_stack.push(make_pair(m_pair.first->m_child[0],m_id));
            }
            else if(m_pair.second==(m_id+1)%3){
                m_stack.push(make_pair(m_pair.first,(m_id+2)%3));
                if(m_pair.first->m_child[1]) m_stack.push(make_pair(m_pair.first->m_child[1],m_id));
            }
            m_pair=m_stack.top();
        }
